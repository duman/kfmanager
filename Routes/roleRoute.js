...
router.post('/', async(req, res) => {
    const {username, rolename, description, requestBy} = req.body; // Get request payload into respective variables
    ...
    */
    ...
    if (!checkValues(req.body, "username|rolename|description")) { // Check if any payload parameters are missing, and if it is throw an error, and quit.
        code = 1;
    ...
    const role_info_specific = await db_each("SELECT username, rolename, description, allowedactions FROM UserRole WHERE username = '" + requestBy + "'");
    ...
    db.run("CREATE TABLE IF NOT EXISTS UserRole (id INTEGER PRIMARY KEY, username TEXT, rolename TEXT, description TEXT, allowedactions TEXT)");

    const role_info = await db_each("SELECT username, rolename, description, allowedactions FROM UserRole");
    ...
    db.run("INSERT INTO UserRole (username, rolename, description, allowedactions) VALUES ('" + username + "', '" + rolename + "', '" + description + "', '" + JSON.stringify(allowedactions) + "')");
